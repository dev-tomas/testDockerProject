...
<body class="white-bg">
    <table>
        <tr>
            <td>Estimado {{ $sale->customer->description }}, le recordamos que está próxima a vencer la {{ $sale->type_voucher->description }} {{ $sale->serialnumber }}-{{ $sale->correlative }},
                por el monto de {{ $sale->coin->symbol }} {{ $sale->credito->debt }}; vence el día {{ date('d-m-Y', strtotime($sale->expiration)) }}. 
                Por favor evite reportes a las centrales de riesgo. Nuestras cuentas bancarias para pago son las siguientes:</td>
        </tr>
        <tr><td><br><br></td></tr>
        <tr>
            <td>
