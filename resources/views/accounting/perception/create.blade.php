...
@section('content')
    <form method="post" role="form" data-toggle="validator" id="frm_perception">
        <div class="row">
            <div class="col-12">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-12 col-md-4">
                                <label for="typeoperation">Percepción</label>
                                <select name="regime" id="regime" class="form-control">
                                    @foreach($regimes as $r)
                                        <option value="{{$r->id}}" rate="{{$r->rate}}">{{$r->description}} - {{$r->rate}}%</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 col-md-2">
                            ...
                            <div class="row">
                                <div class="col-12 table-responsive">
                                    <table class="table" id="tbl_sales">
                                        ...
                                        <th width="100px">MONTO AFECTADO A PERCEPCIÓN</th>
                                        <th width="100px">IMPORTE PERCEPCIÓN</th>
                                        <th width="100px">IMPORTE COBRADO</th>
                                        <th width="10px">*</th>
                                        </thead>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control no-perceived" name="no_perceived[]">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control received" name="received[]">
                                            </td>
                                            <td>
                                                <input type="text" class="form-control amount-charged" name="amount_charged[]">
                                            </td>
                                            <td>
                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <table class="table">
                                            ...
                                            <tr>
                                                <td>Percepción</td>
                                                <td><input type="text" class="form-control" id="perceived" name="perceived" readonly></td>
                                            </tr>
                                            <tr>
                                                <td>Total</td>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary-custom btn-block" id="btnSavePerception">GRABAR PERCEPCIÓN</button>
                            </div>
                        </div>
                    </div>
                </div>
...
toastr.error('Ocurrio un error');
        ...
        function recalculate()
        {
            ...
            $('#tbl_sales tbody tr').each(function() {
                ...
                let amount_charged = total * 1 + rate_final * 1;

                $(tr).find('.total').val(total);
                $(tr).find('.issue').val(issue);
                $(tr).find('.no-perceived').val(total);
                $(tr).find('.received').val(rate_final);
                $(tr).find('.amount-charged').val(amount_charged);



                total_final += total * 1;
                perceived_final += rate_final;
                subtotal += amount_charged;
            });

            $('#subtotal').val(total_final.toFixed(2));
            $('#perceived').val(perceived_final.toFixed(2));
        ...
        $('#btnAddVoucher').click(function() {
            ...
            tr += '<td><input type="text" class="form-control no-perceived" name="no_perceived[]"></td>';
            tr += '<td><input type="text" class="form-control received" name="received[]"></td>';
            tr += '<td><input type="text" class="form-control amount-charged" name="amount_charged[]"></td>';
        ...
        $('#frm_perception').on('submit', function(e) {
            if(e.isDefaultPrevented()) {
                toastr.info('Debe completar todos los campos');
            } else {
                e.preventDefault();
                let data = $('#frm_perception').serialize();
                ...
                $.ajax({
                    url: '/perception/store',
                    type: 'post',
                    data: data + '&_token=' + '{{ csrf_token() }}',
                    dataType: 'json',
                    beforeSend: function() {
                        $('#btnSavePerception').attr('disabled', true);
                    },
                    complete: function() {
                    ...
                    success: function(response) {
                        ...
                        setTimeout(function() {
                            window.location = '/perceptions';
                        }, 5000);
                    },
                    error: function(response) {
                        console.log(response.responseText);
toastr.error('Ocurrio un error');
                        $('#btnSavePerception').removeAttr('disabled');
                    }
                });
            }
        });
